...
//  if (conn_.GetHttpProtocol().raw_url.find("http://") == 0 ||
//      conn_.GetHttpProtocol().raw_url.find("ftp://") == 0) {
//    std::size_t skiped_type_p = conn_.GetHttpProtocol().raw_url.find("//") + 2;
...
//    conn_.GetHttpProtocol().host = config_pool->upstream;
